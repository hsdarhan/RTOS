 ...
 ****************************************************************************/
 ...
 /*
 * Names:
 *   1.  uWaterloo User ID:  *Removed for privacy reasons*
 *   2.  uWaterloo User ID:  hsdarhan@uwaterloo.ca
 */
...
*               to main.c or bst.c.
*****************************************************************
*  1.Indented declarations - void bst_init() - to enable formatting consistency 
*  2.Moved braces to underneathe the if statement - bool bst_insert() - to enable brace consistency
*  3.Removed braces for the "one line" if statement - S32 bst_max() - to eliminate un-needed braces and maintaining consistency
*  4.Moved the command of the if statement to the next line void - void bst_delete() - for better readability and maintaining consistency
*  5.Indented if statements - void bst_delete() - for better readability and maintaining consistency
****************************************************************/
...
*****************************************************************
*  1. bst_delete() - For the case 2 there were reversed assignment of parent node to the current node's right child node
*                    Found by putting a breakpoint at the start of Case 2, and stepping through the code to see that the parent node's incorrect left/right nodes were being assigned
*  2. bst_erase() -  Incorrectly adding 1 to the size of the tree after deleting a node
*                    Found by observing the size variable on the watch window after each step-over iterations from a breakpoint for delete operations
*  3.	bst_delete() - For the case 3 conditional statement, the single '&' was used instead of '&&'(Bitwise vs logical)
*									 - Found by placing a breakpoint in the function, stepping through and making a visual observation.
*  4. bst_delete() - For the case 3 while-loop used for finding the max value, p_swapNode was incorrectly assigned to p_swapNode->left
*                    instead of p_swapNode->right. -Found by setting a breakpoint in the function, stepping through and watching the p_currNode->val
* 									 variable in the call stack window
****************************************************************/
